        ...
        <div class="row">
            <div class="col-md-2 mb-4">
                <select name="type" id="type" class="form-control">
                    ...
                    <option value="pd_lab">गर्भावस्था निदान प्रयोगशाला (Pregnancy Diagnosis Laboratory)
                        ({{count($pdlab)}})</option>
                    <option value="cryo_vessel_blocks">क्रायो-वेसल ब्लॉक (Cryo-Vessel Blocks) ({{count($cvblocks)}})
                    </option>
                    ...
                    <option value="lc_agency">सीमेन बैंक / क्षेत्रीय केंद्र (Semen Bank / Zonal Centers) ( {{$agency}} )
                    </option>
...
<scrpt src="https://cdnjs.cloudflare.com/ajax/libs/js-marker-clusterer/1.0.0/markerclusterer_compiled.js">
    </script>
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBgqN0p57wDl0yxjmUHftcU0QM5h3MGAwM&callback=myMap">
    </script>
    ...
    var newIcon = {
        url: "{{ asset('') }}mapicon/newMaitri.png",
        size: new google.maps.Size(50, 50),
        origin: new google.maps.Point(0, 0),
    };
        ...
        if (val) {
            $.ajax({
                ...
                success: function(data) {
                    if (data.district) {
                        $('#get_district').show();
                        const defaultOption = $('<option></option>').attr('value', '').text(
                            'Select District');
                        ...
                        $.each(district, function(index, item) {
                            ...
                            const option = $('<option></option>')
                                ...
                                .text(item.janpad_name + ': VH(' + item.vh_count +
                                    ') - LEO(' + item.leo_count + ')');
            ...
            for (let i = 0; i < locations.length; i++) {
                if (locations[i]['longitute'] !== "" && locations[i]['lattitute'] !== "") {
                    const contentString =
                        ...
                        " पता : <b>" + locations[i]['janpad_name'] + "," + locations[i]['mandal_name'] +
                        "   </b>,</br>" +
                    ...
                    var icons = (locations[i]['type'] == 'AI' || locations[i]['type'] == 'VH' || locations[i][
                        'type'
                    ] == 'विहि' || locations[i]['type'] == 'V.O.') ? aiimg : vhimg;
            ...
            for (let i = 0; i < locations.length; i++) {
                ...
                if (locations[i]['long'] !== "" && locations[i]['latt'] !== "") {
                    const contentString =
                        ...
                        "<p>  प्रजनन योग्य विदेशी और संकर गाय : <b>" + locations[i]['breedable_extic_cattle'] +
                        "</b>,</br> " +
                        "<p>  प्रजनन योग्य भैंसें : <b>" + locations[i]['breeable_buffaloes'] + "</b>,</br> " +
                        "<p>  कुल प्रजनन योग्य गोजातीय जनसंख्या : <b>" + locations[i]['total_breedable_bovine'] +
                        "</b>,</br> " +
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data.data.result, function(i, index) {
                        var janpad_name = (index.janpad_name) ? index.janpad_name : index
                            .name_hindi;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.code) {
                        ...
                        $('#map').show();
                        $('#janpad_count').append($(
                            "<label for='count' class='btn btn-primary'>Total Maitri : " +
                            data.maitri.length + "</label>"));
                        $('#maitriExport').show();
        ...
        if (locations.length > 0) {
            ...
            for (let i = 0; i < locations.length; i++) {
                ...
                if (locations[i]['type'] == 'bull_mother') {
                    console.log(locations[i]);
                    if (locations[i]['district'] == 'हापुड़' || locations[i]['district'] == 'ललितपुर' || locations[
                            i]['district'] == 'बाराबंकी') {
                        var imgcustom = icon3;
                    }
        ...
        if (locations.length > 0) {
            ...
            for (let i = 0; i < locations.length; i++) {
                if (locations[i]['longitutde'] !== "" && locations[i]['lattitude'] !== "") {
                    ...
                    marker = new google.maps.Marker({
                        position: new google.maps.LatLng(locations[i]['lattitude'], locations[i][
                            'longitutde'
                        ]),
                        map: map,
                        // icon: icon2,
