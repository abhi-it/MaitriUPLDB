...
<div x-data="" class="container main-div" style="background-color:white; height: 100%;min-height:380px;">
    ...
    <table id="my-new-table" class="table table-striped  table-responsive table-bordered">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $i }}</td>
                <td>{{ $stockdeo->zoneStockDetails->demand_section }}</td>
                <td>{{ $stockdeo->zoneStockDetails->semen }}</td>
                <td>{{ $stockdeo->zoneStockDetails->semen_type }}</td>
                <td>{{ $stockdeo->zoneStockDetails->banner }}</td>
                <td>{{ $stockdeo->zoneStockDetails->dangler }}</td>
                <td>{{ $stockdeo->zoneStockDetails->standee }}</td>
                <td>{{ $stockdeo->zoneStockDetails->pamphlet }}</td>
                <td>{{ $stockdeo->zoneStockDetails->ai_kit }}</td>
                <td>{{ $stockdeo->zoneStockDetails->container }}</td>
                <td>{{ $stockdeo->zoneStockDetails->container_capacity }}</td>
                <td>{{ $stockdeo->zoneStockDetails->scheme }}</td>
                <td>{{ $stockdeo->zoneStockDetails->bull_ids }}</td>
                <td>{{ $stockdeo->zoneStockDetails->created_at }}</td>
            </tr>
            @php $i++ @endphp
            @endforeach
        </tbody>
