...
class UpdateController extends Controller
{
    ...
    public function addUpdateLatestNews11(Request $request)
    {
        $user = Auth::user()->id;
        $filePath = $request->id ? Latestupdate::where('id', $request->id)->value('document'): null;

        $path = public_path('uploads/latestnews');
        ...
        if ($request->hasFile('document')) {
            ...
            $filePath = 'uploads/latestnews/'.$fileName;
        }
        
        if ($request->id) {
            $data=Latestupdate::where(['id' => $request->id])->update([
                'user_id'     => $user,
                'title'       => $request->title,
                'url'         => $request->url,
                'description' => $request->description,
            ...
            if ($validator->fails()) {
                return redirect()->back()->with('error', ucfirst($validator->errors()->first()));
            }
